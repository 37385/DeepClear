			...
			public void onClick(View view) {
				...
				try{//获取到白名单
					...
					for (ApplicationInfo info : fullAppList) {
						DialogView dv = new DialogView(MainActivity.this,info,(int)(view.getWidth()/1.6),((TextView)view).getTextSize()/1.7f);
					...
					for (ApplicationInfo info : fullAppList) {
						DialogView dv = new DialogView(MainActivity.this,info,(int)(view.getWidth()/1.6),((TextView)view).getTextSize()/1.7f);
						dv.setEnabled(info.packageName.equals(MainActivity.this.getPackageName()) || Utilities.isSystemApp(info));
						fullAppView.add(dv);
					}
				}
